...
Для создания окошка с офертой встроенных методов типа <code>context.PlatformServices.Confirm</code> оказалось недостаточно. Пришлось сверстать свою сырую html-разметку для окна. Вызвать эту сырую разметку нужно было по клику на другую сырую разметку, поэтому пришлось использовать ванильный  JavaScript в инлайн-атрибутах элемента.
